...
# 8  Use t to get the nearest location on the line to the end of vector pnt_vec_scaled ('nearest').
...
def locateWindow(stimROI,testROI):
 # Define line segment of the social window 
            # is stimulus on left or right? 
            #x from origin, y from origin, width from origin, height from origin
            x=testROI[0]
            y=testROI[1]
            width=testROI[2]
            height=testROI[3]
            
            # line length is always 1/3 of the total stim ROI
            lineLength=height/3
            
            # The x-origin of the line will change if the stimulus is on the left or the right
                        
            if(x<=stimROI[0]): # if x postion of test ROI is lower than the stim ROI x position, stim is on the right
#                stimLR='right'
                lineXstart=x+width
            else:
#                stimLR='left'
            ...
            diff=np.abs(y-stimROI[1])
            
            if(diff>=lineLength):
#                stimTB='bottom'
                lineYstart=y+(lineLength*2)
                lineYend=y+height
            else:
#                stimTB='top'
                lineYstart=y
                lineYend=y+lineLength
...
def DividingLine(stimROI,testROI):
            ...
            height=testROI[3]
             ...
             # line length is always 1/2 of the height of the test chamber 
            VPIStartingPoint= height/2
            
            # The width of the VPI line is always teh width of the test chamber 
            if(x<=stimROI[0]): # if x postion of test ROI is lower than the stim ROI x position, stim is on the right
#                stimLR='right'
                VPILineXstart = x
                VPILineXend = (x + (width/2)) 
                
            else:
#                stimLR='left'
                 VPILineXstart = (x + (width/2)) 
                 VPILineXend = (x + width)    
                  
            VPILineYstart = (y + VPIStartingPoint)
            VPILineYend= (y + VPIStartingPoint)
            
            VPILineYstart= VPILineYend
            VPILineStart = np.zeros(2)
            VPILineStart[0] = VPILineXstart
            VPILineStart[1] = VPILineYstart
            
            VPILineEnd = np.zeros(2)
            VPILineEnd[0] = VPILineXend
            VPILineEnd[1] = VPILineYend
            
            
            return(VPILineStart,VPILineEnd)
